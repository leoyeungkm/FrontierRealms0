...
export function initAppearancePreview(canvas, deps) {
  if (!canvas) return;
  _pvDeps = deps || null;
  // preserveDrawingBuffer：toDataURL 截圖才不會讀到已清空的黑畫面（鑄造預覽圖用）
...
/** 鑄造用：正面定格渲染一幀，回傳 PNG dataURL（避免自轉中截到側/背面或空畫面） */
export function capturePreviewDataURL() {
  if (!_pv || !_pv.model) return null;
  const y = _pv.holder.rotation.y;
  _pv.holder.rotation.y = 0;                 // 截圖定格正面
  _pv.renderer.render(_pv.scene, _pv.camera); // 強制當幀渲染 → 緩衝區有內容
  const url = _pv.renderer.domElement.toDataURL('image/png');
  _pv.holder.rotation.y = y;
  return url;
}
